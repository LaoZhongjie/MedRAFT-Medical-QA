...
### **目标**：微调Student模型，学习检索和推理能力。  
### **步骤**：  

  1. **损失函数设计**：  
     - **答案生成损失**：交叉熵损失，比较Student输出与Teacher答案。  
     - **文档相关性损失**：对比损失，鼓励模型关注Oracle文档，忽略Distractor。  
     - **总损失**：加权组合（答案损失0.7 + 相关性损失0.3）。  

  2. **训练循环**：  
     - 每个样本输入：问题 + 文档集。  
     - 输出：生成CoT推理和答案。  
     - 训练轮数：2–3轮（约5000–6000步，视批次）。  
     - 每500步验证一次，监控损失和ROUGE得分。

  3. 对抗训练：  
     - 负样本强化：若预测为正答案，增加惩罚，鼓励“I don’t know”。
     - 正样本优化：奖励与Teacher答案一致的输出。
